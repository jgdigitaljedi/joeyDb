(function () {
  ...
  function stripString(str) {
    return str.toLowerCase().trim().replace(/[.,\/#!$%\^&\*;:{}=\-_`~()]/g, '').replace(/ +(?= )/g, '');
  }
  ...
  Promise.all(xtxts).then(result => {
    if (result && result.length) {
      ...
      fs.writeFileSync(path.join(__dirname, '../src/static/xboxToXboxThreeSixty.json'), JSON.stringify(final, null, 4));
    } else {
